...
namespace AiAlgorithms.racing
{
    public class MixedMoveChooser : IMoveChooser
    {
            ...
            double flagsTakenC = 10000, double distC = 1, double nextFlagC = 1 / 4)
        {
            ...
            Choosers.Add(new RandomMoveChooser(depth, flagsTakenC, distC, nextFlagC));
        }
        ...
        {
            var results = new List<(ICarCommand FirstCarCommand, ICarCommand SecondCarCommand)>();
            debugInfo = "";
            foreach (var chooser in Choosers){
                results.AddRange(chooser.GetCarCommands(nextFlagForFirstCar, nextFlagForSecondCar, raceState, out var debug));
                debugInfo += debug + "\n";
            }
